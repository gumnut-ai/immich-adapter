...
|------|-----------|-------|
| Assets | Upload, download (original + thumbnail), video playback, delete, bulk delete, existence check, statistics, single-asset and bulk metadata edit | Streaming downloads via `StreamingResponse`; video playback streams the `original` variant from CDN with Range/seek support; `DELETE /api/assets` soft-deletes by default and permanently deletes when `force=true`; `PUT /api/assets/{id}` forwards `description`, paired `latitude` + `longitude`, and `dateTimeOriginal` to the Photos API and emits `on_asset_update`; `PUT /api/assets` forwards the same in-scope fields (plus `timeZone` combined with `dateTimeOriginal`) as one homogeneous `change` to the Photos API bulk-update endpoint, chunking over `BULK_CHUNK_SIZE`. `isFavorite`/`rating`/`visibility` are silently ignored on both paths; `livePhotoVideoId` on the single-asset path and `duplicateId` on the bulk path are silently ignored; the bulk path additionally rejects `dateTimeRelative` and standalone `timeZone` with 422 and skips WebSocket emission |
