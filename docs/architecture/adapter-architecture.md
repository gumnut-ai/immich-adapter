...
| Stacks (delete, bulk-delete, remove-asset) | `DELETE /stacks/{id}` dissolves one stack, `DELETE /stacks/{id}/assets/{assetId}` pulls one frame out, and `DELETE /stacks` dissolves many | All forward to the Gumnut SDK and return a bodyless 204; the photos are untouched. Bulk-delete dedupes its id list (preserving request order), fans out one single-stack delete per id under the shared `gather_with_concurrency` bound, and — since neither Immich nor the SDK promises atomicity — lets every started call settle before raising the first error in request order, so a mid-batch failure is deterministic but can leave earlier dissolves committed. Cover clearing, sub-two dissolution, and user-origin exemption stay server-owned. This completes all seven stack REST endpoints; timeline and sync stacking limits below are unchanged |
