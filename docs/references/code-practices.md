...
**Read the file/provenance scalars from the nested `file_data` object, not the deprecated flat top-level fields.** photos-api exposes the group in two shapes under the same `include=file_data` gate: the preferred nested `gumnut_asset.file_data` object (`checksum_sha1`, `file_modified_at`, `file_size_bytes`, `device_asset_id`, `device_id`, `file_created_at`, `checksum`) and the equivalent flat top-level scalars (`gumnut_asset.checksum_sha1`, …). The flat scalars are deprecated and being removed, so read `gumnut_asset.file_data.<field>` guarding `file_data is None` — `file_data` is `None` when `include=file_data` isn't requested, and that guard preserves the legacy fallbacks (empty Immich checksum on a null `checksum_sha1`, null size, the modify-time cascade above). The whole group is gated by the `file_data` include token either way.
