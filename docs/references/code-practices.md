...
- A `Mock` satisfies any `typing.Protocol` structurally, so tests built on Mock fixtures give a Protocol **zero** conformance checking — and while no production call site passes a real SDK object, pyright has nothing to check either. Pin it with an annotated list of the conforming classes (`STACK_ROW_CLASSES: list[type[GumnutStackRow]]` in `tests/unit/utils/test_stack_conversion.py`); pyright then rejects a class that drifts. Derive nothing from `Protocol.__protocol_attrs__` — it is a runtime-only CPython internal and pyright errors on it.
...
- The same trap has three other shapes, all of which pass while pinning nothing. **Equal-by-fixture values**: if every fixture makes a source equal some other value in scope (a stack row's `asset_count` and the number of members resolved for it), swapping one for the other passes the suite — vary them in at least one test. **Trivially-agreeing paths**: a test comparing two rules that are supposed to *differ* must build the input where they diverge, or both fall through to a shared fallback and agree — the timeline-vs-hydrated cover rules part company only for a pin that is both trashed and still a member. **Assertions true by construction**: the bucket's columnar arrays are largely `[None] * asset_count`, so asserting their lengths match can never fail for those columns; pair a value to its own asset instead. In each case, check the test still fails when the defect it names is reintroduced.
...
When a request degrades by skipping some of N items (a stack the timeline can't resolve, an entity a batch drops), log once per request with counts — `"%d of %d …"` plus `extra={"<thing>_count": n, "requested_<thing>_count": total, "sample_ids": ids[:10]}` — not once per item. Most causes of these are systemic rather than per-item, so a per-item record floods precisely when the signal matters, and on an endpoint a client calls repeatedly (timeline buckets, once per month scrolled) it multiplies again. The ratio is also what separates "all of them" from "one of them", which is the first thing triage needs. `resolve_timeline_stacks` carries the worked example across four such paths.
