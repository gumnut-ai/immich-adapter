...
When the shared value needs a **backend read** the converter can't make — it must stay synchronous and I/O-free — split it in two rather than letting each route resolve its own: an async resolver that answers for a whole page at once, and an optional lookup parameter the converter joins against. `AssetResponseDto.stack` is the worked example (`resolve_asset_stack_summaries` in `routers/utils/stack_conversion.py`, joined by `resolve_asset_stack_summary` in `asset_conversion.py`). The lookup being optional is what keeps the one caller that legitimately doesn't resolve — a stack's own members inside a `StackResponseDto` — from having to fake one; it also means **adding a route that emits the DTO without passing a lookup silently ships the field as null**, which no type check catches. Grep the converter's callers, not just the route you're adding, and give each swept route a test that fails when its lookup is dropped — that test is the only guard this pattern has.

Such a resolver should also **degrade rather than propagate** when the enriching read fails, which is the opposite of the posture for a read whose subject *is* the response. The global `GumnutError` handler forwards an upstream status verbatim, so a 404 from an enrichment lookup bolted onto `GET /api/assets/{id}` would answer 404 for an asset that exists — and a route that already chose to tolerate partial failure (`search_memories` tolerates a failed year) would have that choice silently overridden by a decorative field. Catch the SDK's `GumnutError` base class only: an `AttributeError` or a validation failure in the resolver is an adapter bug, and swallowing it converts a visible 500 into a field that is quietly absent on every response.
...
Without the break, a `limit=1` "is this non-empty?" probe on a busy day burns one round-trip per matching asset. Breaking out early leaks nothing — the paginator awaits and fully buffers each page before yielding any of its items, so no response is left open and the break only skips the *next* page's request (the abandoned async generator is closed at GC). A corollary for tests: a helper that counts in-flight reads with `try`/`finally` around its yields measures generator *lifetime*, not concurrency, and never decrements for a consumer that breaks — bracket the awaited round-trip instead.
...
- A guard test asserting two call sites agree must compare the calls they actually make (`call_args.kwargs`), not their source text: `inspect.getsource` + a regex matches the same literal in the **docstring**, which silently made an `order="asc"` parity check vacuous. Whatever the mechanism, prove a new guard *fails* when its subjects disagree — temporarily break one and re-run — since a guard that never fails is worse than none, being mistaken for coverage.
