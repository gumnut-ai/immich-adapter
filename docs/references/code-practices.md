...
4. **Verify parameter semantics**: Check the Immich OpenAPI spec (`https://api.immich.app/endpoints/`) or source code (`immich/server/src/controllers/*.controller.ts` and the matching service) to confirm what each URL path and body parameter represents. URL `{id}` parameters don't always refer to the entity in the URL collection — face/person reassign endpoints in particular swap the natural reading. Both of these accept the **target person** as `{id}` in the path:
   ...
   - The same duty applies to any **claim about client behavior you write down** — in a comment, docstring, test name, or doc — not just to implementation decisions. "Web passes `X`" / "no client reads this field" are plausible-sounding and frequently wrong: one param is typically passed differently across its call sites — `withHidden` is passed both ways, and omitted entirely by some callers — so what one caller does rarely generalizes. Grep the fork for **every** caller before asserting one, and don't commit the resulting tally: it rots on the next fork sync with nothing in CI pinning it. **Refuting** such a claim owes the same grep — "the client doesn't do that" is itself a client-behavior claim, and a review that reaches it from a partial caller list argues confidently for the wrong edit.
   - The spec is also **not the full route surface**: Immich marks some live routes `@ApiExcludeEndpoint`, which omits them from the OpenAPI document and therefore from the generated models, the generated clients, and any spec diff. A spec-driven check can only tell you a replacement endpoint doesn't exist *in the spec* — e.g. `PATCH /stacks/{id}` supersedes the deprecated `PUT` in the v3.1.0 source while being invisible to all of the above. Read the controller when the question is whether a route exists at all.
...
**Chunking is not always the right answer.** Where a partial run can't be undone, refusing the over-cap request is better than half-performing it: let the backend's 422 through and say so in the docstring. `POST /stacks` is the worked example — creating a stack repoints its members out of whatever stacks they were in, so a failure partway can't be compensated by deleting the half-built stack (that dissolves it without restoring what was folded in), and no realistic burst approaches the cap anyway. Decide by asking what a mid-loop failure leaves behind, not by whether the endpoint takes a list.
