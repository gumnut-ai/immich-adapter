...
Use `exif_dims_and_orientation(asset)` from `routers/utils/asset_conversion.py` at every emit site that populates `exifInfo.exifImageWidth/Height` and the EXIF `orientation` field. The helper returns `(exifImageWidth, exifImageHeight, wire_orientation)` and bakes the orientation-nulling rule in:
