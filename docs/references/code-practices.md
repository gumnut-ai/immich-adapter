...
- **Committed text — public names only, no internal references**: This is a public repository, so everything committed (code comments, docstrings, test names and docstrings, design docs under `docs/design-docs/`, `docs/architecture/`, `docs/references/`, `docs/guides/`, plus PR bodies, commit messages, and branch names) must read cleanly for an external reader who can't see anything private. Three things to keep out:
  ...
  - **Internal issue-tracker IDs** (e.g., `GUM-123`). External readers cannot resolve them and they leak the existence of internal work-tracking. Inline the rationale instead so the reasoning is self-contained (e.g., "the recently shipped end-to-end Range path" rather than "GUM-713"). This catches **branch names** too: a pushed branch is publicly visible on the remote.

  Existing references predate this rule; sweep them out opportunistically when editing nearby content. (The literal `photos-api` / `GUM-123` / `GUM-713` tokens in this bullet are deliberate examples of what to avoid.)
...
- **Branching**: Always create a new branch from `main` before making changes. Name it after the change, not after an issue ID — see the internal issue-tracker IDs rule above. Don't modify files on an existing feature branch for unrelated work.
