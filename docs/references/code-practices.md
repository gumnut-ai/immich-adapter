...
- Unit tests call endpoint handlers directly, bypassing FastAPI parameter resolution — a parameter declared `x: bool = Query(default=None)` receives the raw `Query` object (which is truthy), not `None`, so `if x:` silently takes the True branch. Pass such parameters explicitly (e.g., `isTrashed=False`) whenever the test's intent depends on which branch runs; `get_asset_statistics` is the canonical case.
