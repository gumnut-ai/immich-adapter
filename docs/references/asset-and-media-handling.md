...
Use `exif_dims_and_orientation(gumnut_asset)` from `routers/utils/asset_conversion.py` at every emit site that populates `exifInfo.exifImageWidth/Height` and the EXIF `orientation` field. The helper returns `(exifImageWidth, exifImageHeight, wire_orientation)` and renders the orientation-nulling rule in:
...
default `edited=false` streams the **edit base**. The archive route currently
streams the current rendering regardless of `edited`.

The edit base is the highest-position version whose `kind` is not `edit` (or
`edit:*`), selected by `select_edit_base` in `routers/utils/asset_version_chain.py`.
The `edited=false` download and `services/asset_edit_renderer.py` share that one
helper so "the original" means the same version everywhere the adapter exposes
it. Skipping `edit` versions keeps repeated adjustments non-cumulative;
preferring the latest non-edit version keeps an `external:*` rendering layered on
the upload. Until an external rendering exists, the base is the position-0
original. External renderings are produced from the full chain below them, so an edit
below an `external:*` version is already baked in and the latest non-edit is
always the correct base. Note that the Immich web editor previews
`GET /api/assets/{id}/thumbnail?size=preview&edited=false`, and `view_asset`
does not yet honor `edited`, so that preview still shows the current rendering.
