...
Keep codebase-specific facts and traps that a capable contributor could reasonably miss. Do not pad the doc with generic framework, language, HTTP, SQL, or testing tutorials. The same bar applies to agent-facing docs — runbooks, guides, and `AGENTS.md`: state the goal and the constraints a capable agent could not easily work out from the repository, not command recipes or step checklists it can derive. Keep a command when its exact form is itself such a constraint. `README.md` is written for humans and keeps its common commands.
