...
**Effort**: **S remaining** — the shared translation layer (`routers/utils/stack_conversion.py`), the read routes, and the nested stack summaries on the REST asset surfaces have landed; the 5 write routes plus the timeline bucket's stack tuples are what's left.
