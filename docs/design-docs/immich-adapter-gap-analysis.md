...
What has landed splits on reachability, and the asset stack fields are the hinge. Clients discover an existing stack from the stack fields on an asset, which the adapter does not populate yet — so the reads and set-cover are effectively unreachable, since the UI that calls them only appears on an asset the client already believes is stacked. **Create is not gated that way**: the web timeline's multi-select "Stack" action and the duplicates utility page call it directly. So a user can stack from the timeline today, the client optimistically collapses the selection, and the grouping then disappears on refresh because nothing reads back as stacked. That half-working state is the argument for doing the asset stack fields next rather than the removal routes.
...
**Effort**: **S remaining** — the shared translation layer (`routers/utils/stack_conversion.py`), the read routes, and the create/set-cover writes have landed; the 3 removal routes plus the stack fields on the timeline and asset-detail surfaces are what's left.

**Recommendation**: **Revisit later** — the expensive half (translation, cover policy, read routes, create/set-cover) is done, but the remaining work is still low user demand relative to the Tier-1 and Tier-2 gaps.
...
| #6 Stacks (removal path) | S | Adapter-only | Low user demand; reads, the translation layer, and the create/set-cover writes shipped, leaving the 3 removal routes |
