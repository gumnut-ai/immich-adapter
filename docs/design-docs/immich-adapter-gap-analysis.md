...
**Current behavior**: **Partially closed.** The two read endpoints (`GET /stacks`, `GET /stacks/{id}`) return real stacks with their live members, the list bounded by a 500-stack cap and a 5000-member hydration budget (whichever binds first) since Immich's `searchStacks` offers no pagination. Every REST `AssetResponseDto` now also carries its nested `stack` summary — asset detail and update, upload success, search, and memories — which is how a client discovers a stack in the first place. The other 5 endpoints — create, update-cover, delete, bulk-delete, remove-asset — still return empty/fake responses, so the stacking UI can display a stack but not edit one.

**User impact**: **Low** — Stacking is a power-user feature. Most users don't manually stack photos. Some Immich features auto-create stacks (e.g., for live photos), but the adapter handles live photos differently. The REST asset surfaces now populate the nested stack block clients discover a stack from, so the landed reads are reachable: the burst badge renders and the viewer's stack controls appear. Two paths still report no stacks, and they are what a user would notice next — the timeline bucket's own `stack` tuples, so a collapsed burst still occupies one tile per frame, and the sync stream's `SyncAssetV1.stackId`, which is how the Immich **mobile** client builds its asset store, so mobile sees no stacks at all regardless of what REST reports.
...
**Effort**: **S remaining** — the shared translation layer (`routers/utils/stack_conversion.py`), the read routes, and the nested stack summaries on the REST asset surfaces have landed; the 5 write routes, the timeline bucket's stack tuples, and the sync stream's `SyncAssetV1.stackId` are what's left. The sync one is the least mechanical: the summary path's resolver is request-scoped, and the sync stream would need its own.
...
| #6 Stacks (write path) | S | Adapter-only | Low user demand; reads, the translation layer, and the nested asset stack summaries shipped, leaving the 5 write routes and the timeline tuples |
